...
// Подпись на кнопке — русская, для тебя. В API уходит query — всегда
// английский, поиск заточен под аудиторию США (regionCode/relevanceLanguage
// в youtubeApi.ts), русский запрос почти ничего не найдёт в этой настройке.
const PRESET_NICHES: { label: string; query: string }[] = [
  { label: 'Заработок на AI', query: 'AI side hustle 2026' },
  { label: 'Стройка на публику', query: 'build in public' },
  { label: 'AI-дропшип', query: 'AI dropshipping' },
  { label: 'Пассивный доход онлайн', query: 'passive income online' },
  { label: 'No-code AI инструменты', query: 'no code AI tools' },
  { label: 'Путь соло-основателя', query: 'solo founder journey' }
];
...
export default function Dashboard() {
  ...
  async function handleSearch(overrideQuery?: string, displayLabel?: string) {
    // Если в поле остался русский текст с кнопки-пресета (не ретайпнутый
    // руками) — при повторном "Найти"/Enter резолвим его обратно в
    // английский запрос, а не шлём русский текст в API заново.
    const matchedPreset = PRESET_NICHES.find(n => n.label === keyword);
    const q = overrideQuery ?? (matchedPreset ? matchedPreset.query : keyword);
    if (!q.trim()) return;
    if (overrideQuery) setKeyword(displayLabel ?? overrideQuery);
    setLoading(true);
    setExpandedId(null);
    ...
    try {
      ...
      if (!res.ok || data.error) {
        // Раньше ошибка тут молча превращалась в пустой список — выглядело
        // как "ничего не нашлось", хотя на деле сервер вернул ошибку
        // (например, YOUTUBE_API_KEY не задан на Vercel).
        setSearchError(data.error || `Сервер ответил ошибкой ${res.status}`);
        setIdeas([]);
      } else {
        setIdeas(data.ideas || []);
      }
    } catch (err: any) {
      setSearchError(err.message || 'Ошибка сети');
      setIdeas([]);
    } finally {
      setLoading(false);
    }
  ...
  async function handleShowChannels() {
    ...
    if (channels.length === 0) {
      ...
      try {
        ...
        const data = await res.json();
        if (!res.ok || data.error) {
          setChannelsError(data.error || `Сервер ответил ошибкой ${res.status}`);
        } else {
          setChannels(data.channels || []);
        }
      } catch (err: any) {
        setChannelsError(err.message || 'Ошибка сети');
      } finally {
  ...
  return (
    <div className="container">
      ...
      <div style={{ display: 'flex', flexWrap: 'wrap', gap: 8, marginBottom: 16 }}>
        {PRESET_NICHES.map(n => (
          <button
            ...
            onClick={() => handleSearch(n.query, n.label)}
            disabled={loading}
          >
            {n.label}
          </button>
      ...
      {showChannels && (
        <div style={{ marginBottom: 20 }}>
          ...
          {!channelsLoading && channelsError && (
            <div className="missing-data">Ошибка: {channelsError}</div>
          )}
      ...
      {searchError && !loading && (
        <div className="missing-data" style={{ marginBottom: 16 }}>Ошибка поиска: {searchError}</div>
      )}

      {ideas.length === 0 && !loading && !searchError && (
